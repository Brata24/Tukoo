<script lang="ts">
	...
	async function exportToExcel() {
		...
		try {
			...
			script.src = 'https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.full.min.js';
	...
	async function exportToPDF() {
		...
		try {
			...
			// Prepare table data
			const tableBody: any[][] = [
				['No', 'Product Name', 'Category', 'Qty Sold', 'Total Revenue', 'Orders']
			];

			data.salesReport.forEach((item, index) => {
				tableBody.push([
					String(index + 1),
					item.productName,
					item.categoryName,
			...
			// Add summary row
			tableBody.push([
				{ text: 'TOTAL', colSpan: 3, bold: true } as any,
				{} as any,
				{} as any,
				{ text: String(data.summary.totalItems), bold: true } as any,
				{ text: formatCurrency(data.summary.totalRevenue), bold: true } as any,
				{ text: String(data.summary.totalOrders), bold: true } as any
			]);
			...
			const docDefinition = {
				...
				content: [
					{ text: data.merchant.name, style: 'header' },
					{ text: `Sales Report - ${monthName} ${data.selectedYear}`, style: 'subheader' },
					{ text: `Generated: ${new Date().toLocaleDateString('id-ID')}`, style: 'date' },
					{ text: '\n' },
					{
						style: 'tableStyle',
						table: {
				...
				styles: {
					...
					tableStyle: {
						fontSize: 9,
						margin: [0, 5, 0, 15] as any
					}
				}
			};

			pdfMake.createPdf(docDefinition).download(`Sales_Report_${monthName}_${data.selectedYear}.pdf`);
			
			// Clean up
			document.head.removeChild(pdfMakeScript);
			document.head.removeChild(vfsScript);
		} catch (error) {
